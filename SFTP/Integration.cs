...
namespace SFTP
{
    public class Integration
    {
        ...
        public Integration(ILogger<Integration> logger, SftpConfig sftpConfig)
        {
            _logger = logger;
            _config = sftpConfig;
        }
        ...
        public void UploadFile(string remoteFilePath, byte[] bytes)
        {
            using var client = new SftpClient(_config.Host, _config.Port == 0 ? 22 : _config.Port, _config.UserName, _config.Password);
            try
            {
                client.Connect();
                client.WriteAllBytes(remoteFilePath, bytes);
